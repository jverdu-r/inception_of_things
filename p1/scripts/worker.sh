...
# Esperar a que el servidor K3s esté completamente listo
echo "Waiting 90 seconds for master server to be fully operational..."
sleep 90

# Leer y limpiar el token correctamente
TOKEN=$(cat /vagrant/token.env | head -n 1 | tr -d '[:space:]')

# Creating an environment variable for installing the worker node
# https://docs.k3s.io/installation/configuration#configuration-file

if export INSTALL_K3S_EXEC="agent --server https://192.168.56.110:6443 -t $TOKEN --node-ip=192.168.56.111"; then
